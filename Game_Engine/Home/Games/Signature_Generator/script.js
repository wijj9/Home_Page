var r, e, t, a = r=>e=>{
    ...
    , l = a(r=>Q(r)), o = a(r=>1 / (1 + N(-r))), n = a(r=>Q(1 + N(r))), v = a(r=>{
        var e = N(2 * r);
        return (e - 1) / (e + 1)
    }
), i = r=>(e,t)=>{
    ...
    , u = i((r,e)=>r + e), f = i((r,e)=>r - e), s = i((r,e)=>r * e), h = i((r,e)=>r / e), d = r=>{
    var e = Y(K(r))
        , t = 0;
    for (let a = 0; a < K(r); a++)
        e[a] = N(r[a]),
    ...
    for (let r = 0; r < p; r++) {
        var w = d[(r - 1 + p) % p]
            ...
            , x = j(m, T(M, .2))
            , C = I(g, T(y, .2))
            , A = "C " + sr(x[0]) + " " + sr(x[1]) + ", " + sr(C[0]) + " " + sr(C[1]) + ", " + sr(g[0]) + " " + sr(g[1]);
        c.push(A)
    }
    var k = c.join(" ")
    ...
    if (0 != K(a)) {
        ...
        for (let n = r; n < K(o); n++) {
            a = o[n];
            K(a) < 2 ? 0 != K(a) && 1 != a[0][2] || (r += 1) : (e && lr.removeChild(lr.lastChild),
                q(a, t, l),
                1 == a[K(a) - 1][2] ? (e = !1,
                    r += 1) : e = !0)
        }
    ...
    , j = (r,e)=>[e[0] + r[0], e[1] + r[1]], I = (r,e)=>[r[0] - e[0], r[1] - e[1]], T = (r,e)=>[e * r[0], e * r[1]], V = (r,e,t)=>{
    var a = 0
        , l = []
        , o = []
        , n = [];
    ...
    w: 24,
    y: 22
}, N = Math.exp, P = Math.sqrt, Q = Math.log, R = Math.random, W = (Math.abs,
    Math.max,
    Math.min,
    Math.floor), X = P(.5), G = 256, J = 512, K = r=>r.length, O = (r,e,t)=>r.slice(e, t), Y = function() {
    return new Float32Array(...arguments)
}, Z = null, $ = $, rr = null, er = null, tr = [], ar = document.getElementById.bind(document), lr = ar("canvas"), or = ar("select-style"), nr = ar("bias-slider"), vr = ar("speed-slider"), ir = ar("width-slider");
ir.oninput = ()=>(r=>{
        for (; lr.lastChild; )
            lr.removeChild(lr.lastChild);
        if (0 != K(r)) {
...
ur = "https://d33au9p5d8tjhf.cloudfront.net/d.bin",
                        ...
                        , l = r=>{
                            do {
                                ...
                                for (let r = 0; r < s; r++)
                                    h[r] = a.getUint16(e, !0),
                                        e += 2;
                                ["y", "w", "r", "l"].includes(n) ? u = V(u, f, h) : v && (u = _(u, f, h)),
                                    t[n] = u
                            } while (performance.now() - r < 16 && e < a.byteLength);
                            e < a.byteLength ? window.requestAnimationFrame(l) : (ar("draw-button").addEventListener("mousedown", E),
    ...
    ar("save-button").addEventListener("click", ()=>{
            ...
            l.setAttribute("href", "data:image/svg+xml;base64," + window.btoa(a)),
                l.setAttribute("download", Z.toString().toLowerCase().replace(/\s+/g, "-").replace(/[^\w\-]+/g, "").replace(/\-\-+/g, "-").trim() + ".svg"),
                l.click()
        }
    );    var r, e, t, a = r=>e=>{
    var t = Y(K(e));
